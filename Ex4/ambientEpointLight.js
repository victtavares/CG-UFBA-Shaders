...
function init() {
	...
	ambientLight = new THREE.AmbientLight(new THREE.Color(0.15, 0.15, 0.15));
	scene.add(ambientLight);
	
	pointLight = new THREE.PointLight(new THREE.Color(1.0, 1.0, 1.0));
	pointLight.distance = 10.0;
	pointLight.position.set(0.0, 0.8, 0.0);
...
function initGUI() {
	...
	controls = new function () {
		...
		this.lightPosX      = pointLight.position.x;
		this.lightPosY 		= pointLight.position.y;
		this.lightPosZ 		= pointLight.position.z;
	...
	gui.addColor(controls, 'pointLightColor').onChange(function (value) {
		pointLight.color = new THREE.Color(controls.pointLightColor);
		sphereLightMesh.material.color = new THREE.Color(value);
		});
	gui.addColor(controls, 'ambientLightColor').onChange(function (value) {
	...
	fLightPos.add( controls, 'lightPosX', -1.0, 1.0).onChange(function (value) {
		pointLight.position.x = sphereLightMesh.position.x = controls.lightPosX;
		});
	fLightPos.add( controls, 'lightPosY', -1.0, 1.0).onChange(function (value) {
		pointLight.position.y = sphereLightMesh.position.y = controls.lightPosY;
		});
	fLightPos.add( controls, 'lightPosZ', -1.0, 1.0).onChange(function (value) {
		pointLight.position.z = sphereLightMesh.position.z = controls.lightPosZ;
		});
	fLightPos.close();
