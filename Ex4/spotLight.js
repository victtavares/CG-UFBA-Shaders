...
function init() {
	...
	spotLight = new THREE.SpotLight(new THREE.Color(1.0, 1.0, 1.0));
	spotLight.distance = 10.0;
	spotLight.position.set(0.0, 0.8, 0.0);
	spotLight.angle = Math.PI/2;
	scene.add(spotLight);

	// Fonte de luz - representacao geometrica
	var sphereLight = new THREE.SphereGeometry(0.01);
	var sphereLightMaterial = new THREE.MeshBasicMaterial(new THREE.Color(0.4, 0.0, 0.5));
...
function initGUI() {
	...
	controls = new function () {
		...
		this.lightPosX      = spotLight.position.x;
		this.lightPosY 		= spotLight.position.y;
		this.lightPosZ 		= spotLight.position.z;
		this.lightColor		= spotLight.color;
		}

	var gui = new dat.GUI();
	
	gui.add(controls, 'angle', 0.0, Math.PI/2).onChange(function (value) {
	...
	gui.addColor(controls, 'lightColor').onChange(function (value) {
		spotLight.color = new THREE.Color(controls.lightColor);
	...
	fLightPos.add( controls, 'lightPosX', -1.0, 1.0).onChange(function (value) {
		spotLight.position.x = sphereLightMesh.position.x = controls.lightPosX;
		});
	fLightPos.add( controls, 'lightPosY', -1.0, 1.0).onChange(function (value) {
		spotLight.position.y = sphereLightMesh.position.y = controls.lightPosY;
		});
	fLightPos.add( controls, 'lightPosZ', -1.0, 1.0).onChange(function (value) {
		spotLight.position.z = sphereLightMesh.position.z = controls.lightPosZ;
		});
	fLightPos.close();
